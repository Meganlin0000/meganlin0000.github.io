...
18. **去7化(2026-07-13,團隊指示)**:正式網址=**https://ladybug-tku.vercel.app**(ladybug.vercel.app 被占,取 TKU=淡江;舊 7adybug.vercel.app 仍指向同站不會死);本機資料夾改 `~/Desktop/ladybug-site/`(launch.json 同步,埠 5599 名稱 ladybug);信箱=ladybug.7.tku@gmail.com(**formsubmit 要用新信箱重新點啟用信!**);IG=ladybug.7_;hero 加 WhyUs 便利貼;刪為什麼找學生團隊/三方案/安心區;about 文案=我們是 Ladybug…讀音中藏著 Debug 的巧思。
19. **失敗信 100% 斷根(2026-07-13)**:①ladybug repo 的 Pages 已 DELETE(備援網址 /ladybug/ 失效,沒差);②她 GitHub 帳號通知設定 Actions 的 Email 管道已關(只留 on GitHub)。四道防線:兩 repo 自訂 workflow 停用+ladybug Pages 刪除+Actions email 關閉。她再收到=新問題。
20. IG 帳號更正=ladybug.7__(兩條底線)。
